...
import {
  buildClusterUtilizationDemoData,
  buildDashboardClusterRecentActivitiesPreview,
...
const CLUSTER_METRICS = [
  {
    key: 'cpu',
    title: 'CPU usage',
    subtitle: 'vCPU utilization across running cluster nodes',
    dataKey: 'cpu' as const,
    stroke: VM_UTILIZATION_CHART_STROKES.cpu,
  },
  {
    key: 'memory',
    title: 'Memory usage',
    subtitle: 'Memory pressure across worker nodes in your clusters',
    dataKey: 'memory' as const,
    stroke: VM_UTILIZATION_CHART_STROKES.memory,
  },
  {
    key: 'gpu',
    title: 'GPU usage',
    subtitle: 'Accelerator load on GPU-enabled cluster node pools',
    dataKey: 'gpu' as const,
    stroke: VM_UTILIZATION_CHART_STROKES.gpu,
  },
  {
    key: 'storage',
    title: 'Storage usage',
    subtitle: 'Persistent volume utilization across cluster workloads',
    dataKey: 'storage' as const,
    stroke: VM_UTILIZATION_CHART_STROKES.storage,
  },
] as const

export type DashboardUtilizationVariant = 'virtual-machines' | 'clusters'
...
export function DashboardVmUtilizationSection({
  ...
  demoTenantId,
  variant = 'virtual-machines',
}: {
  isDarkTheme: boolean
  onOpenRecentActivities: () => void
  fleetVirtualMachines: readonly TenantVirtualMachine[]
  demoTenantId: DemoTenantId
  variant?: DashboardUtilizationVariant
}) {
  const [period, setPeriod] = useState<VmUtilizationPeriod>('7d')
  const [periodMenuOpen, setPeriodMenuOpen] = useState(false)
  const isClusterVariant = variant === 'clusters'
  const metrics = isClusterVariant ? CLUSTER_METRICS : VM_METRICS
  const utilizationHeading = isClusterVariant
    ? 'Cluster utilization trends'
    : 'VM utilization trends'
  const sectionAriaLabel = isClusterVariant
    ? 'Cluster utilization trends and recent activities'
    : 'VM utilization trends and recent activities'

  const data = useMemo(
    () =>
      isClusterVariant
        ? buildClusterUtilizationDemoData(period, demoTenantId)
        : buildVmUtilizationDemoData(period, fleetVirtualMachines),
    [isClusterVariant, period, fleetVirtualMachines, demoTenantId],
  )
  const activities = useMemo(
    () =>
      isClusterVariant
        ? buildDashboardClusterRecentActivitiesPreview(demoTenantId, 6)
        : buildDashboardVmRecentActivitiesPreview(fleetVirtualMachines, demoTenantId, 6),
    [isClusterVariant, fleetVirtualMachines, demoTenantId],
  )
